...
async def test_register_webhook_success():
    ...
    with respx.mock:
        ...
        respx.get("https://api.github.com/repos/testowner/testrepo/hooks").mock(
            return_value=httpx.Response(200, json=[])
        )

        # Mock successful webhook creation
        respx.post("https://api.github.com/repos/testowner/testrepo/hooks").mock(
...
async def test_register_webhook_repo_not_found():
    ...
    with respx.mock:
        respx.get("https://api.github.com/repos/testowner/nonexistent/hooks").mock(
                ...
                json=[
                    {
                        ...
                        "config": {"url": "https://stonehaven.example.com/webhook/test-project"},
                    },
                    {"id": 22222222, "config": {"url": "https://different.example.com/webhook"}},
                ],
            )
        )
...
async def test_register_webhook_includes_secret():
    ...
    with respx.mock:
        ...
        webhook_request = respx.post("https://api.github.com/repos/testowner/testrepo/hooks").mock(
            return_value=httpx.Response(201, json={"id": 12345678})
        )
...
async def test_register_webhook_update_existing_fails():
    ...
    with respx.mock:
        ...
        with pytest.raises(
            WebhookError, match=f"Failed to update existing webhook {existing_webhook_id}"
        ):
            await register_webhook(
                repo_owner="testowner",
                repo_name="testrepo",
...
async def test_register_webhook_create_fails_with_validation_errors():
    ...
    with respx.mock:
        ...
        with pytest.raises(
            WebhookError, match=r"Validation Failed.*config.url.*is invalid.*events.*is too long"
        ):
            await register_webhook(
                repo_owner="testowner",
                repo_name="testrepo",
...
def test_register_webhook_sync():
    ...
    with respx.mock:
        ...
        # Mock successful webhook creation
        respx.post("https://api.github.com/repos/testowner/testrepo/hooks").mock(
            return_value=httpx.Response(201, json={"id": 87654321})
        )
...
async def test_register_webhook_proper_headers():
    ...
    with respx.mock:
        ...
        hooks_request = respx.get("https://api.github.com/repos/testowner/testrepo/hooks").mock(
            return_value=httpx.Response(200, json=[])
        )

        create_request = respx.post("https://api.github.com/repos/testowner/testrepo/hooks").mock(
            return_value=httpx.Response(201, json={"id": 12345678})
        )
