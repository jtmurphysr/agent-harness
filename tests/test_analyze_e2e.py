...
from cli.init import InitError, init_analyze, init_finalize
...
def mock_analysis_result():
    ...
    result.proposed_context = {
        ...
        "becoming": ["Add monitoring and observability", "Implement caching layer"],
        "reviewers": {
...
""")
    ...
    (temp_project_root / "README.md").write_text(
        "# Test Project\nA FastAPI application for testing."
    )

    with (
        patch("cli.init.RepoAnalyzer") as mock_analyzer_class,
...
""")
    ...
    with (
        ...
        auth_related = any(
            "auth" in inv["rule"].lower() or "auth" in inv["id"].lower() for inv in invariants
        )
        input_related = any(
            "input" in inv["rule"].lower() or "valid" in inv["rule"].lower() for inv in invariants
        )
        assert auth_related or input_related, (
            "Should include security-related invariants for web apps"
        )
...
async def test_analyze_e2e_pr_creation(
    temp_project_root, github_token, mock_claude_client, mock_analysis_result, mock_subprocess
):
    ...
    with (
        ...
        mock_subprocess_run.side_effect = [
            ...
            MagicMock(
                stdout="git@github.com:testowner/testrepo.git", returncode=0
            ),  # git remote get-url
...
def test_analyze_e2e_finalize_after_merge(
    temp_project_root, mock_templates_dir, stonehaven_url, github_token, mock_subprocess
):
...
@pytest.mark.asyncio
async def test_analyze_e2e_not_git_repo_error(temp_project_root, github_token, mock_claude_client):
...
async def test_analyze_e2e_analysis_failure_cleanup(
    ...
    with patch("cli.init.RepoAnalyzer") as mock_analyzer_class:
        ...
        mock_analyzer.analyze_repository = AsyncMock(
            side_effect=AnalysisError("Analysis failed due to unreadable code")
        )
...
async def test_analyze_e2e_pr_creation_failure_cleanup(
    ...
    with (
        ...
        assert not factory_dir.exists(), (
            ".factory directory should be removed on PR creation failure"
        )


def test_finalize_missing_factory_dir_error(
    temp_project_root, mock_templates_dir, stonehaven_url, github_token
):
