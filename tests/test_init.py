...
from cli.init import InitError, init_analyze, init_finalize, init_greenfield
...
def test_init_greenfield_missing_context(
...
def test_init_greenfield_invalid_context(
    temp_project_root, templates_dir, stonehaven_url, github_token
):
...
def test_extract_repo_info_git_failure_fallback():
    ...
    project_context = {"project": {"name": "owner/repo-from-context"}}
...
def test_extract_repo_info_no_git_no_slash():
    ...
    project_context = {"project": {"name": "just-a-name"}}
    ...
    with patch("cli.init.httpx.Client") as mock_client_class:
        ...
        with pytest.raises(InitError) as exc_info:
            _register_with_stonehaven(
                "https://stonehaven.test", "test-id", "owner/repo", "test-project"
            )
...
def test_extract_repo_info_weird_github_url():
    ...
    with tempfile.TemporaryDirectory() as temp_dir, patch("subprocess.run") as mock_run:
        ...
        mock_run.return_value.stdout = (
            "ftp://github.com"  # Contains github.com but not proper format
        )
        mock_run.return_value.returncode = 0
...
def mock_analysis_result():
    ...
    result.proposed_context = {
        "project": {"name": "Analyzed Project", "description": "A project analyzed by Claude"},
        ...
        "invariants": [{"id": "test_invariant", "rule": "Test rule", "severity": "correctness"}],
        "sharp_edges": [],
        "structural_decisions": [],
        "becoming": [],
        "reviewers": {
...
async def test_init_analyze_creates_context_pr(
    ...
    with (
        patch("interview.analyzer.RepoAnalyzer") as mock_analyzer_class,
