...
""")
        ...
        assert (
            "FastAPI" in context["stack"]["framework"]
            or "fastapi" in context["stack"]["framework"].lower()
        )
...
"""
    ...
    def test_should_exclude(self, analyzer):
        ...
        assert analyzer._should_exclude(
            Path("/test/repo/node_modules/package/index.js"), repo_path, test_patterns
        )
        assert analyzer._should_exclude(
            Path("/test/repo/__pycache__/main.pyc"), repo_path, test_patterns
        )
        ...
        assert not analyzer._should_exclude(
            Path("/test/repo/src/utils.py"), repo_path, test_patterns
        )
    ...
    def test_identify_databases(self, analyzer):
        ...
        sqlite_package = {"name": "requirements.txt", "content": "sqlite3\nsqlalchemy>=1.4.0"}
        ...
        mongo_package = {"name": "requirements.txt", "content": "pymongo>=4.0.0\nmotor>=2.5.0"}
    ...
    def test_identify_deployment_type(self, analyzer):
        ...
        cli_info = {"key_files": ["pyproject.toml"], "languages": ["Python"], "frameworks": []}
    ...
    def test_analysis_result_dataclass(self):
        ...
        result = AnalysisResult(
            proposed_context=context, confidence_score=0.85, analysis_notes="Test analysis notes"
        )
