...
from cli.sync import TemplateSyncer, TemplateUpdate
...
async def test_sync_fleet_no_updates_needed(syncer):
    ...
    with (
        patch.object(syncer, "_get_current_template_versions") as mock_current,
        ...
        assert result == {"updated": [], "skipped": ["owner/project1", "owner/project2"]}
...
async def test_sync_fleet_single_template_upgrade(syncer):
    ...
    with (
        ...
        assert result == {"updated": ["owner/project1"], "skipped": []}
...
async def test_sync_fleet_pr_creation(syncer):
    ...
    with respx.mock:
        ...
        respx.post("https://api.github.com/repos/owner/project1/git/refs").mock(
            httpx.Response(201, json={"ref": "refs/heads/factory/template-upgrade-123456"})
        )

        # 4. Get current templates_lock.yml
        current_lock_content = yaml.safe_dump(
            {"engineer.template.md": "1.1.0", "architect.template.md": "2.0.0"}
        )
        import base64
        ...
        respx.post("https://api.github.com/repos/owner/project1/pulls").mock(
            httpx.Response(
                201, json={"number": 42, "html_url": "https://github.com/owner/project1/pull/42"}
            )
        )
        ...
        await syncer._create_template_upgrade_pr("owner/project1", [update], "test-token")
...
def test_find_template_updates(syncer):
    ...
    updates = syncer._find_template_updates(
        project_lock, current_versions, "owner/repo", "Test Project"
    )
...
def test_create_commit_message(syncer):
    ...
    assert single_message == "Update engineer.template.md from v1.0.0 to v1.1.0"
