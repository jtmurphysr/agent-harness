...
def sample_project(client: VerdictStoreClient) -> int:
    ...
    return client.create_project(
        stonehaven_id="test-uuid-1234", repo="testorg/testrepo", project_name="Test Project"
    )
...
class TestVerdictStoreClient:
    ...
    def test_create_project_success(self, client: VerdictStoreClient) -> None:
        ...
        project_id = client.create_project(
            stonehaven_id="uuid-1234", repo="testorg/testrepo", project_name="Test Project"
        )
    ...
    def test_write_findings_batch(self, client: VerdictStoreClient, sample_verdict: int) -> None:
    ...
    def test_get_verdicts_for_project_empty(
        self, client: VerdictStoreClient, sample_project: int
    ) -> None:
    ...
    def test_get_findings_for_verdict_empty(
        self, client: VerdictStoreClient, sample_verdict: int
    ) -> None:
    ...
    def test_transaction_rollback_on_error(self, client: VerdictStoreClient, temp_db: Path) -> None:
        ...
        valid_findings = [
            FindingRecord(
                verdict_id=verdict_id, bucket="bad", text="Valid finding", severity="WARN"
            )
        ]
    ...
    def test_write_verdict_lastrowid_none(
        self, client: VerdictStoreClient, sample_project: int
    ) -> None:
