...
def test_webhook_handler_valid_signature(
    webhook_handler: WebhookHandler, sample_payload: bytes, webhook_secret: str
) -> None:
    """Test webhook handler accepts valid HMAC signature."""
    expected = create_valid_signature(sample_payload, webhook_secret)

    # Patch the placeholder secret for testing. Parameter names match the real
    # method so the replacement has the same signature under mypy.
    webhook_handler._verify_signature = lambda payload, signature, secret: signature == expected

    result = webhook_handler._verify_signature(sample_payload, expected, webhook_secret)
...
def test_webhook_handler_missing_sha256_prefix(
    ...
    result = webhook_handler._verify_signature(
        sample_payload, signature_without_prefix, webhook_secret
    )
    assert result is False
...
def test_webhook_handler_duplicate_delivery_id(
    ...
    # Patch verification to always pass for this test, so no real signature is needed
...
def test_webhook_handler_unknown_repo(
    webhook_handler: WebhookHandler, sample_payload: bytes
) -> None:
...
def test_webhook_endpoint_missing_signature_header(test_client: TestClient) -> None:
    ...
    response = test_client.post(
        "/webhook", headers={"X-GitHub-Delivery": "test-delivery"}, content=b"test payload"
    )
...
def test_webhook_endpoint_missing_delivery_header(test_client: TestClient) -> None:
    ...
    response = test_client.post(
        "/webhook", headers={"X-Hub-Signature-256": "sha256=test"}, content=b"test payload"
    )
...
async def test_webhook_endpoint_invalid_signature_returns_401(test_client: TestClient) -> None:
    ...
    response = test_client.post(
        "/webhook",
        headers={"X-Hub-Signature-256": "sha256=invalid", "X-GitHub-Delivery": "test-delivery"},
...
async def test_webhook_endpoint_duplicate_delivery_returns_409(
    test_client: TestClient, webhook_handler: WebhookHandler
) -> None:
    ...
    response = test_client.post(
        "/webhook",
        headers={"X-Hub-Signature-256": "sha256=test", "X-GitHub-Delivery": delivery_id},
        content=b"test payload",
    )
...
async def test_webhook_endpoint_processing_error_returns_200(
    ...
    response = test_client.post(
        "/webhook",
        headers={"X-Hub-Signature-256": "sha256=test", "X-GitHub-Delivery": "test-error-delivery"},
        content=b"test payload",
    )
...
def test_webhook_endpoint_response_time_constraint(test_client: TestClient) -> None:
    ...
    response = test_client.post(
        "/webhook",
        headers={"X-Hub-Signature-256": "sha256=invalid", "X-GitHub-Delivery": "timing-test"},
        content=b"test payload",
    )
